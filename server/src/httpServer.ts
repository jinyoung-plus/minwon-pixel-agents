...
/** True if `addr` is an IPv4/IPv6 loopback literal. The server binds to
 *  127.0.0.1 by default, so this exempts only genuinely local clients
 *  (e.g. browser tabs opening minwon_multiagent.html that POST to
 *  /api/hooks/:providerId without a Bearer token). */
...
function bearerAuth(expectedToken: string) {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    // Loopback bypass: requests from the local machine are allowed without a
    // Bearer token. Mirrors the WebSocket route's standalone-mode auth bypass
    // (the WS route skips auth entirely when not embedded; the hook route is
    // stricter and only skips when the remote address is actually a loopback).
