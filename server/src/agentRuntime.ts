  ...
  constructor(
    ...
    this.hookEventHandler.setLifecycleCallbacks({
      onExternalSessionDetected: (sessionId, transcriptPath, cwd) => {
        ...
        // hooks-only sessions (no transcript file -- the cwd is a free-form label
        // chosen by the external client, e.g. minwon's role name) bypass the
        // tracked-project-dir gate. They can never collide with a real workspace
        // scan and the hook route already restricts ingress to loopback.
        const hooksOnly = !transcriptPath;
