  ...
  useEffect(() => {
    ...
    const handler = (msg: any) => {
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        // hooks-only externals (e.g. minwon roles) spawn in IDLE wander mode;
        // they only sit down when a PreToolUse event activates them.
        const hooksOnly = msg.hooksOnly === true;
        ...
        if (isTeammate && teammateParentId !== undefined) {
          ...
          os.addAgent(id, undefined, undefined, undefined, undefined, folderName, hooksOnly);
        }
        saveAgentSeats(os);
