...
export class OfficeState {
  ...
  addAgent(
    ...
    // startIdle agents (hooks-only externals such as minwon's roles) are NOT
    // pre-seated -- they spawn on a random walkable tile and wander the floor
    // until their first PreToolUse fires setAgentActive(true), which then
    // lazily claims a free seat and the FSM walks them to it.
    ...
    if (startIdle) {
      // Drop out of createCharacter's TYPE+isActive default into the IDLE
      // wander loop. PreToolUse will flip isActive back on.
      ch.isActive = false;
  ...
  setAgentActive(id: number, active: boolean): void {
    const ch = this.characters.get(id);
    if (!ch) return;
    // Lazy seat assignment for startIdle agents: the first time they become
    // active, claim a free seat (findFreeSeat prefers PC-facing ones) so the
    // IDLE→WALK→TYPE FSM walks them to a desk. Seat is retained across
    // subsequent active/idle cycles so the same role keeps the same desk.
    if (active && !ch.seatId && !ch.isSubagent) {
      const seatId = this.findFreeSeat();
      if (seatId) {
        const seat = this.seats.get(seatId)!;
        seat.assigned = true;
