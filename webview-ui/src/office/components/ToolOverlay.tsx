  ...
  return (
    <>
      {allIds.map((id) => {
        ...
        // "Done" agents (finished turn with checkmark) hide the big panel so the
        // floating green bubble stays visible — unless the user explicitly
        // hovers or selects them.
        const isDone = ch.bubbleType === 'waiting' && !ch.waitingAwaitingInput;
        const showBigPanel = isSelected || isHovered || (alwaysShowOverlay && !isDone);

        // When the big panel is hidden, fall back to a small always-on name chip
        // so external roles (e.g. minwon's 6 agents) remain identifiable at a
        // glance even with no hover/select and the user setting turned off.
        if (!showBigPanel) {
          if (!ch.folderName) {
            return (
              <div
          ...
          return (
            ...
            >
              <span
                className="pixel-panel px-4 py-1 leading-none whitespace-nowrap inline-block"
                style={{ fontSize: '14px' }}
                data-testid="agent-name-chip"
              >
                {ch.folderName}
              </span>
            </div>
